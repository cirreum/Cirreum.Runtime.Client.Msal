...
public abstract class CommonGraphProfileEnricher(
	...
	public async Task EnrichProfileAsync(UserProfile profile, ClaimsIdentity identity) {
		...
		if (Tid.IsEmpty()) {
			const string warn = "Unable to get the tid from the ClaimsPrincipal. Ensure the ClaimsPrincipal was created from an Msal/EntraID provider.";
			logger.LogWarning(warn);
			return;
		}
		var Oid = identity.GetObjectId();
		if (Oid.IsEmpty()) {
			const string warn = "Unable to get the oid from the ClaimsPrincipal. Ensure the ClaimsPrincipal was created from an Msal/EntraID provider.";
			logger.LogWarning(warn);
			return;
		}
		...
		try {
			...
			logger.LogError(ode, "OData error querying the Microsoft Graph.");
		} catch (ServiceException se) {
			logger.LogError(se, "Graph Service error querying the Microsoft Graph.");
		} catch (Exception e) {
			logger.LogError(e, "Unknown error querying the Microsoft Graph.");
		}
